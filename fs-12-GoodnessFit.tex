...
\begin{frame}
  ...
  for categorical distributions in a moment. First, however, we will
  have a look at the Kolmogorov-Smirnov test, which tests the goodness
  of fit with respect to a continuous distribution.
\end{frame}
...
\begin{figure}[h]
\includegraphics[scale=.5]{./diagrams/KS_Example.png}
\end{figure}
\end{frame}
...
\begin{frame}[fragile]
  ...
  hypothesis (which is always: yes, the data was generated from the
  continuous distribution under investigation) passes the test. If the
  IQ results on the next page are in the data set \texttt{iq}, then
  the command in R Statistics to perform the Kolmogorov-Smirnov test
  is
\begin{alltt}
