...
\begin{frame}[fragile,fragile]
  ...
  \texttt{summary(lm(d{\textasciitilde}y))}. The output is on the next slide. Notice
...
\begin{frame}
  ...
  \texttt{z<-summary(lm(d{\textasciitilde}y))}). The residual standard deviation
...
\begin{frame}
  ...
  \begin{itemize}
  \item In the R summary the coefficient of determination is called
    ``Multiple R-squared'' and equals $r^{2}$. The reason why it is
    squared is because one could say that the correlation accounts for
    $r^{2}$ of the variation in the $y$-values.
  \item $r^{2}$ is called the \alert{coefficient of determination}. In
    the Nenana example, which year it is accounts for 9.8\% of the
    variation in the number of days it takes for the ice to break.
  \item Remember that knowing $r^{2}$ does not tell you whether $r$ is
    positive or negative---you can look at the slope of the regression
    line for that information.
  \item Some statisticians prefer ``Adjusted R-squared'' which
    penalizes larger numbers of parameters.
  \end{itemize}
\end{frame}
...
b<-rnorm(500,a,0.2) \newline
plot(a,b) \newline
abline(lm(b{\textasciitilde}a),col="red")
\end{alltt}
\end{frame}
...
\texttt{summary(lm(faithful[[2]]{\textasciitilde}faithful[[1]]))}. 
\end{frame}
...
The R Statistics command \texttt{confint(lm(y{\textasciitilde}x),'x',level=0.95)} will
...
\begin{frame}
  ...
  The R Statistics command \texttt{\footnotesize predict(lm(y{\textasciitilde}x),data.frame(x=4),level=0.95,interval="predict")}
...
% \begin{alltt}
% confint(lm(faithful[[2]]{\textasciitilde}faithful[[1]]),'faithful[[1]]',level=0.95)
% \end{alltt}
